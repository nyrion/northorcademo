...
/**
 * @classdesc Nyrion Canary. Reference by window.canary.
 *
...
window.canary = {
    ...
    config: {
        cyndar: { // Nyrion Cyndar
            apiKey: "AIzaSyDNJPfyLY5agzKAWVaUnk6dHQCK44jSqMc",
            authDomain: "nyrion-mobile.firebaseapp.com",
            databaseURL: "https://nyrion-mobile.firebaseio.com",
            projectId: "nyrion-mobile",
            storageBucket: "nyrion-mobile.appspot.com",
            messagingSenderId: "823922161206",
            appId: "1:823922161206:web:c5abde7473f7babc870a97",
            measurementId: "G-D5PP78Y2KC"
        }
    },
    ...
    initialize: function() {
        if ( !window.canary.device ) {
            ...
            else if ( /Switch|3DS/i.test( navigator.userAgent ) ) device = "switch";
            else if ( /Xbox/i.test( navigator.userAgent ) ) device = "xbox";
            else if ( /PlayStation/i.test( navigator.userAgent ) ) device = "ps4";
            else device = "web";
        ...
        if ( !window.canary.ready && window.canary.config && window.firebase && window.firebase.performance && window.firebase.analytics && window.firebase.firestore ) {
            window.firebase.initializeApp( window.canary.config["cyndar"] );
